...
- [x] Add a research-needed import lane for Notion pages with real request content but no delivery path — `applyImportReadinessGuard()` now routes those cases to `needs_contact_info` with `MISSING_DELIVERY_PATH`, and `applyImportDeliveryFallback()` now uses the intake placeholder email so those imports persist instead of failing `email_or_portal_required` `(TESTED VIA REGRESSION+LIVE REPLAY - Codex 2026-03-09 - notion-sync-guard.test.js, 28 passing; fresh replay on :3113 imported previously failing page 21387c20-070a-8114-a32c-f887d32cc48e as case #26649)`
- [x] Suppress raw Notion relation IDs from import text/metadata mismatch detection — relation-type properties are now omitted from `formatAllPropertiesAsText()` / `preparePropertiesForAI()`, so `AGENCY_METADATA_MISMATCH` only keys off real agency names `(TESTED VIA REGRESSION+LIVE PAGE CHECK - Codex 2026-03-09 - request-normalization.test.js + notion-sync-guard.test.js; page 21d87c20-070a-80f0-b6aa-dca1a10a8fb7 no longer emits relation IDs or a mismatch hint)`
- [x] Normalize loose imported incident dates before case creation — textual dates like `February 2022` now normalize to a DB-safe `YYYY-MM-DD` calendar date instead of crashing insert `(TESTED VIA REGRESSION+LIVE REPLAY - Codex 2026-03-09 - notion-sync-guard.test.js, 28 passing; case #26649 stored `incident_date::date = 2022-02-01`)`
- [x] Apply AI-resolved agency names during single-page Notion import — `processSinglePage()` now preserves `aiResult.agency_name`, which unblocks correct agency/contact lookup for narrative-only pages with no direct PD relation `(TESTED VIA REGRESSION+LIVE TRACE - Codex 2026-03-09 - notion-sync-guard.test.js, 29 passing; page 21387c20-070a-8114-a32c-f887d32cc48e now resolves to "Jacksonville Sheriff's Office (JSO)" before contact lookup)`
- [x] Remove stale false-positive `AGENCY_METADATA_MISMATCH` warnings caused by stored Notion relation IDs on already-imported Samuel cases `(TESTED VIA LIVE DB CLEANUP - Codex 2026-03-09 - cleared warnings from cases 26639, 26640, 26641, 26642, 26643, 26644, 26645; mismatch count across the 10-case Samuel batch is now 0)`
- [x] Clean up placeholder single-page imports instead of leaving bad live proposals behind — case `#26635` now has its stale proposal dismissed, stale Lubbock `case_agencies` mapping deactivated, `PLACEHOLDER_TITLE` + `MISSING_REQUEST_CONTENT` warnings attached, and the case is parked in `needs_human_review` with no pending proposal `(TESTED VIA LIVE API+DB - Codex 2026-03-09 - /api/requests/26635/workspace shows Unknown agency, no portal, placeholder warnings, and no active proposal)`
- [x] Clean up wrong-agency single-page imports and re-dispatch with corrected delivery metadata — case `#26649` was repaired from the stale Lubbock mapping to `Jacksonville Sheriff’s Office, Florida`, stale proposal `#1949` dismissed, and a fresh Jacksonville proposal `#1959` generated after re-dispatch `(TESTED VIA LIVE API+DB - Codex 2026-03-09 - /api/requests/26649/workspace now shows Jacksonville agency/email/portal and proposal #1959 as PENDING_APPROVAL)`
...
- [ ] Fix review/control-state reconciliation for cleanup-parked imports — case `#26635` is correctly parked with no pending proposal, but workspace still reports `control_state=OUT_OF_SYNC` / `decision_required_without_pending_proposal` after dismissing the stale proposal
- [ ] Tighten fuzzy agency matching so generic city names do not beat the actual agency match on single-page imports — `findAgencyByName("Jacksonville Sheriff's Office", 'FL')` still returned the generic `Jacksonville` record instead of `Jacksonville Sheriff’s Office, Florida`, which is why case `#26649` needed manual correction
...
- [x] Fix gated queue inbound-context fallback when the trigger email is linked by `thread_id` / `trigger_message_id` but missing `messages.case_id` — `live-overview` now falls back to the proposal trigger message for `last_inbound_*` and `inbound_count`, and historical inbound trigger messages were backfilled onto their cases (`158` rows updated, including case `#25148` / message `#990`) `(TESTED VIA LIVE DB+CODE REVIEW - Codex 2026-03-09)`
- [x] Fix gated queue previews for HTML-only inbound emails and stop exemption denials from defaulting to fake “narrowing” rebuttals — `live-overview` now derives inbound preview text from `body_html` when `body_text` is empty, and `generateDenialRebuttal()` now uses HTML fallback plus subtype inference so cited surveillance/privacy denials route as `privacy_exemption` instead of falling back to `overly_broad` `(TESTED VIA LOCAL SMOKE - Codex 2026-03-09 - case #25157 denial HTML now extracts cleanly and infers privacy_exemption)`
- [x] Canonicalize inbound message storage and reads across DB/UI/AI — added `messages.normalized_body_text`, `normalized_body_source`, and `is_substantive`; normalization now runs on write + historical backfill; shared DB getters now return canonical body text while preserving `raw_body_text`; secondary readers (gated/detail/export/shadow/PDF-form/Trigger safety checks) now use the normalized field instead of independently re-parsing `body_text` vs `body_html` `(TESTED VIA DB+UNIT+PIPELINE+UI - Codex 2026-03-09 - 1,218 messages normalized, 0 proposal/thread-linked messages missing case_id, case #25148 inbound now renders in app, case #25157 inbound HTML now renders in app and fresh rebuttal generation no longer narrows)`
- [x] Add inbound integrity guardrails for future regressions — reconciliation report now exposes `inbound_linkage_gaps`, `empty_normalized_inbound`, and `proposal_message_mismatches`; system health card exposes the same metrics with drill-down endpoints; cron operational alerts now watch the three counters in a rolling 1h window `(TESTED VIA ROUTE+UNIT+API+BUILD - Codex 2026-03-09 - quality-report-service.test.js + system-health-routes.test.js passing; /api/eval/reconciliation and /api/monitor/system-health on :3134 expose the new metrics; dashboard build passes with new health-card/reconciliation types)`
- [ ] Triage live inbound linkage gaps surfaced by the new guardrails — current fresh server on `:3134` shows 2 unlinked Warren County “Submission Confirmation” inbound messages (`#724`, `#709`) in `/api/eval/reconciliation` and `/api/monitor/system-health/details?metric=inbound_linkage_gaps`
