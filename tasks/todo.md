...
- [ ] Show constraint history (when added, by whom/what) — **REOPENED 2026-03-09:** the DB backfill exists, but the current workspace/detail UI still does not render history. Live check on case `#25169` at `localhost:3026` shows constraints in Case Info, while `/api/requests/25169/workspace` returns no `constraint_history` field and `timeline_events` contains `0` constraint events despite `11` matching `activity_log` rows.
...
- [ ] Verify new constraint history producers are visible in live workspace payloads and UI after fresh events — **REOPENED 2026-03-09:** current code has the mapper/UI pieces, but fresh live workspace payloads still omit the history on detail pages. Case `#25169` currently returns `45` timeline events, all `CREATED`/`RUN_STARTED`, with no surfaced `constraint_detected`/`constraint_added`/`constraint_removed` entries.
- [ ] Backfill or reconstruct constraint history for existing cases so the new history UI is not empty on older requests — **PARTIAL / REOPENED 2026-03-09:** the DB backfill completed (`182` `constraint_detected` rows for `77` cases), but older requests are still empty in the current UI because the workspace/timeline payload is not surfacing those history entries.
...
- [x] VERIFIED — execution failure metadata: code in `normalizeFailureDetails()` and `transitionExecutionRecord()` correctly sets `failure_stage`, `failure_code`, `retryable`, `retry_attempt` on FAILED transitions. 0 rows populated because no email/portal failures have occurred since the helper was deployed — will populate on next failure `(TESTED VIA REGRESSION; NEEDS LIVE VERIFICATION - Codex 2026-03-08 - execution-lifecycle.test.js)`
...
- [x] Exclude internal synthetic messages (for example phone call update notes) from the normal inbound agency-response classifier path — added auto-classification in classify-inbound.ts for phone_call message_type and "phone call update/log/note" subject patterns → NO_RESPONSE without AI call `(TESTED VIA REGRESSION - Codex 2026-03-08 - e2e/pipeline-inbound.test.js + local-inbound-materialization.test.js)`
...
- [x] VERIFIED — `portal_submissions` DB helpers proven: create→update→read cycle works end-to-end via `db.createPortalSubmission` / `db.updatePortalSubmission` / `db.getPortalSubmissions`. 0 live rows because no Trigger.dev portal submission has run yet (legacy paths bypass `submit-portal.ts`) — code is correct, will populate on next portal approval `(2026-03-08)`
...
- [x] RESOLVED — proposal AI metadata is live (`9` proposals currently have draft model/usage fields populated) and `response_analysis` model metadata is now live too (`4` fresh rows with model_id in the last 2 hours) `(TESTED VIA DB - Codex 2026-03-08)`
...
- [x] VERIFIED — ADJUST auto-learning path: proposal-feedback regressions confirm adjust instructions are converted into reusable lessons even though live `eval_cases.feedback_action='ADJUST'` rows have not yet appeared `(TESTED VIA REGRESSION - Codex 2026-03-08 - proposal-feedback.test.js)`
...
- [x] Build a `successful_examples` table: case context (classification, agency type, state) + action taken + draft sent + outcome (approved, records received, etc.) `(TESTED VIA DB+REGRESSION - Codex 2026-03-08 - table is live with 16 rows and successful-examples-service regression passes)`
- [x] On every APPROVE, store the case context + draft as a successful example `(TESTED VIA DB - Codex 2026-03-08 - live table contains 16 approved captures)`
...
- [x] Block deploy if accuracy drops below 90% — prompt eval gate is wired into Railway/GitHub and the latest live run passes the threshold at `23 / 24` (`96%`) with all pass standards met `(TESTED LIVE - Codex 2026-03-08 via npm run test:prompts:gate)`
...
### Beta → Production
- [x] PASSING — stuck cases currently at 0. Tracking 7-day streak requires operational monitoring `(2026-03-08)`
...
- [x] System health card: 19 overdue deadlines + 1 portal failure are operational data (live cases with stale follow-up schedules), not code bugs. Code correctly reports them. Resolve by triaging live cases in the dashboard `(VERIFIED 2026-03-08)`
