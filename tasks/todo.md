...
- [x] Show constraint history (when added, by whom/what) `(TESTED VIA UI+DB - Codex 2026-03-08 - case detail renders history and live DB contains 182 backfilled constraint_detected entries)`
...
- [x] Fresh-backend prompt E2E harness now runs past case creation and through the real inbound pipeline when forced to a clean local API base URL; representative fixtures `denial_weak`, `portal_redirect_simple`, `multi_portal_plus_fee`, and `direct_question` all pass on localhost:3070 after the `createEmailThread` fallback patch and the `/api/requests/:id` response-shape fix in the harness `(TESTED LOCALLY - Codex 2026-03-08)`
- [x] E2E prompt case creation path no longer fails on the old DB `ON CONFLICT` error; fresh localhost runs now create the case, ingest inbound mail, dispatch Trigger, and materialize proposals through the expected endpoints (`POST /api/cases`, `POST /api/cases/:id/ingest-email`, `POST /api/cases/:id/run-inbound`) `(TESTED LOCALLY - Codex 2026-03-08)`
