...
- [ ] Case completion report: requested vs received — **REOPENED 2026-03-09:** route handler exists in `routes/requests/case-management.js`, but the live stack still returns `404 Not found` for `GET /api/requests/:id/completion-report` on both backend `3025` and the dashboard proxy.
...
- [ ] Email-to-case: forward article link to special address, auto-create case — **REOPENED 2026-03-09:** regression coverage exists, but fresh live API checks still fail. `POST /api/cases/email-intake` returns `404 Cannot POST` on backend `3025`, and on backend `3028` it reaches the handler but fails with `new row for relation "cases" violates check constraint "email_or_portal_required"`, so the end-to-end intake path is not yet live-proven.
...
- [ ] Auto-escalate priority when deadlines approach — **REOPENED 2026-03-09:** the unit test still passes, but the current live DB has `0` `priority_auto_escalate` events and `4` cases inside the cron’s actual 3-day `deadline_date` window still below urgent priority (`#25175`, `#25204`, `#25205`, `#25206`).
...
| Portal Tasks | `/portal-tasks/` | FAIL | Current UI renders 0 pending / 0 in progress / 0 completed / 0 failed / 0 cancelled with the empty-state message "All portal tasks are complete", but `/api/portal-tasks` on the same stack returns 1 live `PENDING` task. This looks like a status-casing bug in the page filters. |
