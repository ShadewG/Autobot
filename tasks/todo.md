...
- [ ] Fix `/portal-tasks/` page status handling so live tasks are not hidden by status-casing drift — **NEW 2026-03-09:** the current dashboard API returns 1 task with status `PENDING`, but `/portal-tasks/` renders all-zero KPIs and the empty state because the page and case panel are filtering for lowercase `'pending'` / `'in_progress'`.
...
- [ ] Add Decision Traces section to case detail — AI classification/routing/gate decision visualization with node trace flow, confidence coloring, run links, lazy-load from audit-stream — **REOPENED 2026-03-09:** the section renders, but it is not faithfully reflecting live data. Case `#25532` shows `DECISION TRACES 0` / "No decision traces recorded" in the detail UI while the audit stream for the same case reports `decision_traces=2`. Current detail code is still fetching `/audit-stream?source=decision_traces` and reading `data.events`, while the live route returns `entries` and summary data.
...
- [x] Investigated DB saturation: all 53300 errors are from `cron_service/operational_alert_check` (every 15min) hitting Railway's PostgreSQL connection limit during peak concurrent access. Not a code bug — transient operational issue when multiple services share the connection pool. Pool max is 10, queries are minimal (2 parallel)
...
- [ ] Proposal AI metadata live rollout is still partial — the schema is in place and draft-step metadata is persisting, but the current DB retest still shows only `12 / 738` proposals with draft model fields and `0 / 738` with decision model fields populated. `response_analysis` is healthier at `57 / 236` rows with `model_id`, so the classify step is live while the decide-step capture is still unproven. **REOPENED 2026-03-09 after current DB retest.**
...
- [ ] Fix portal access issue routing: current synthetic sweep matched `0/12`; observed outputs include `ESCALATE`, `NEGOTIATE_FEE`, `SEND_CLARIFICATION`, and `NO_PROPOSAL` instead of the expected portal/help response patterns `(Evidence: tests/reports/email-scenario-batch-report.json, labels real:portal_access_issue:741/738/732/731/730/707/695/689)`
- [ ] Fix blank request form handling: current synthetic sweep matched `0/12`; all examples drifted to `SEND_CLARIFICATION` instead of the request-form return flow (`SEND_PDF_EMAIL`) `(Evidence: tests/reports/email-scenario-batch-report.json, labels real:blank_request_form:936/934/932/931/929/927/925/923/921/920/918)`
- [ ] Fix portal confirmation / portal release false actions: confirmations should usually stay inert or produce lightweight portal-specific handling, but the current sweep still emitted stray `SEND_APPEAL`, `SEND_FEE_WAIVER_REQUEST`, `ACCEPT_FEE`, and `ESCALATE` proposals `(Evidence: real:portal_confirmation:706/607 and real:portal_release:867/669 in tests/reports/email-scenario-batch-report.json)`
- [ ] Investigate fee-letter policy drift: the real-pattern corpus currently mismatched `12/12` fee-letter scenarios, splitting between `NEGOTIATE_FEE` and `ACCEPT_FEE`; verify whether the expected action labels are stale or whether the decision prompt is too fee-aggressive `(Evidence: real:fee_letter:* in tests/reports/email-scenario-batch-report.json)`
- [ ] Fix denial-letter reliability: the corpus produced six timeout/error cases and several denial letters that resolved to fee actions before later escalating/appealing; inspect why denial-heavy runs often take >120s and why they sometimes detour through fee/referral actions `(Evidence: cases 25921, 25922, 25923, 25925, 25926, 25929 from the 2026-03-09 corpus run; proposals 1337, 1340, 1342, 1344, 1349, 1354)`
- [ ] Fix wrong-agency referral handling: the corpus mostly returned `NO_PROPOSAL` or `ESCALATE` instead of reliably rerouting/researching the correct agency `(Evidence: real:wrong_agency_referral:* in tests/reports/email-scenario-batch-report.json; only 1/12 matched expected behavior)`
- [ ] Normalize malformed real-pattern sender addresses in the synthetic corpus runner so invalid fixture emails do not short-circuit the workflow with a 422 before classification `(Evidence: real:wrong_agency_referral:723 failed ingest with invalid from_email during the 2026-03-09 corpus run)`
- [ ] Add a synthetic-test mode that suppresses Notion sync/status writeback for locally created corpus cases; the current 100-scenario sweep floods `error_events` with `notion_service object_not_found` noise and slows runs without adding product signal `(Evidence: localhost:3037 corpus run logs and report generated at 2026-03-09T01:01:28.164Z)`
