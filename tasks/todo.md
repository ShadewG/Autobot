...
- [x] Fix portal confirmation / portal release stale expected labels: msg 607 ($444 fee) → `pay_fee`, msg 867 ($50 fee) → `pay_fee`, msg 669 (delivery) → `download`. Remaining: msg 706 (juvenile_records denial expected=wait but AI produces SEND_APPEAL) is a decision quality issue, not fixture — **FIXED 2026-03-09**
...
- [x] Fix wrong-agency referral expected labels: updated stale `respond` → correct actions: phone redirect (723) → `find_correct_agency`, Iowa DCI redirect (685) → `find_correct_agency`, cancelled requests (591/490/447) → `wait`. Remaining entries (770/769/700/698/489/486) already had correct labels. AI routing quality (NO_PROPOSAL/ESCALATE vs RESEARCH_AGENCY) remains separate — **FIXED 2026-03-09**
