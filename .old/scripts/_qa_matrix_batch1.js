...
async function runScenario(scenario, decision = null) {
  const caseId = await createSeed({ name: 'Synthetic QA Records Unit, Arizona' });
  ...
  return {
    ...
    logs: (finalRun.logs || []).slice(-10)
  };
}

(async () => {
  const results = [];
  results.push(await runScenario('fee_low_auto'));
  results.push(await runScenario('denial_weak'));
  results.push(await runScenario('fee_high_approve', { action: 'APPROVE' }));
  results.push(await runScenario('fee_high_withdraw', { action: 'WITHDRAW' }));
