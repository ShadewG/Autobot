...
function resolveReviewState({ caseData, activeProposal, activeRun }) {
    ...
    const importWarningTypes = new Set(
        (Array.isArray(caseData?.import_warnings) ? caseData.import_warnings : [])
            .map((warning) => String(warning?.type || '').trim().toUpperCase())
            .filter(Boolean)
    );
    const isImportReviewPlaceholder = /placeholder notion page|imported case/i.test(substatus);
    const hasBlockingImportWarnings = [
        'AGENCY_NOT_IN_DIRECTORY',
        ...
        'MISSING_DELIVERY_PATH',
        'MISSING_EMAIL',
        'NO_MX_RECORD',
    ].some((type) => importWarningTypes.has(type));
    const isManualPhoneHandoff =
        caseStatus === 'needs_phone_call' &&
        !proposalStatus;
    const isManualContactInfoHandoff =
        caseStatus === 'needs_contact_info' &&
        !proposalStatus;
    const isManualReadyToSendHandoff =
    ...
    const hasDecisionGateFromCaseFlags =
        requiresHuman &&
        !NON_DECISION_HUMAN_PAUSES.has(pauseReason) &&
        !isImportReviewPlaceholder &&
        !hasBlockingImportWarnings &&
        !isManualPhoneHandoff &&
        !isManualContactInfoHandoff &&
        !isManualReadyToSendHandoff;
    ...
    if (REVIEW_REQUIRED_CASE_STATUSES.has(caseStatus)) {
        ...
        if (
            (((caseStatus === 'needs_human_review' || caseStatus === 'needs_human_fee_approval') &&
                (NON_DECISION_HUMAN_PAUSES.has(pauseReason) || isManualReadyToSendHandoff || isImportReviewPlaceholder || hasBlockingImportWarnings)) ||
                isManualPhoneHandoff ||
                isManualContactInfoHandoff) &&
            !proposalStatus
        ) {
