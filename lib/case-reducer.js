...
const CaseEvent = Object.freeze({
  ...
  CASE_RESPONDED:       'CASE_RESPONDED',
  CASE_WRONG_AGENCY:    'CASE_WRONG_AGENCY',
  FEE_QUOTE_RECEIVED:   'FEE_QUOTE_RECEIVED',

  // Classification-driven
  ACKNOWLEDGMENT_RECEIVED:    'ACKNOWLEDGMENT_RECEIVED',
  PARTIAL_DELIVERY_RECEIVED:  'PARTIAL_DELIVERY_RECEIVED',
...
function computeMutations(snapshot, event, context = {}) {
  ...
  switch (event) {
    ...
    case CaseEvent.RUN_FAILED: {
      m.cases = {
        ...
        requires_human: true,
        pause_reason: context.pauseReason || 'AGENT_RUN_FAILED',
    ...
    case CaseEvent.PROPOSAL_GATED: {
      m.cases = {
        status: 'needs_human_review',
        requires_human: true,
        pause_reason: context.pauseReason || 'PENDING_APPROVAL',
      };
      if (context.runId) {
        m.agent_runs = { id: context.runId, status: 'waiting' };
      }
    ...
    case CaseEvent.PROPOSAL_DISMISSED: {
        m.proposals = {
          ...
          human_decision_merge: {
            dismissal_type: 'system_auto_dismiss',
    ...
    case CaseEvent.PROPOSAL_WITHDRAWN: {
      m.cases = {
        ...
        pause_reason: null,
        substatus: context.substatus || 'proposal_withdrawn'
      };
    ...
    case CaseEvent.PROPOSAL_BLOCKED: {
      m.cases = {
        ...
        ...(context.error ? { substatus: String(context.error).substring(0, 100) } : {}),
      };
    ...
    case CaseEvent.EMAIL_SENT: {
      ...
      // Dismiss other active proposals (awaiting_response is in the clear list)
      Object.assign(m, proposalDismissForStatus('awaiting_response', 'case_status:awaiting_response'));
      break;
    }
    ...
    case CaseEvent.PORTAL_COMPLETED: {
      ...
      m.portal_tasks = {
        id: context.portalTaskId,
        status: 'COMPLETED',
        completed_at: new Date().toISOString(),
        ...(context.completedBy ? { completed_by: context.completedBy } : {}),
        ...(context.confirmationNumber ? { confirmation_number: context.confirmationNumber } : {}),
      };
    ...
    case CaseEvent.PORTAL_FAILED: {
      ...
      // Dismiss the portal proposal so it doesn't block dispatch of fallback actions
      if (context.proposalId) {
        m.proposals = { id: context.proposalId, status: 'DISMISSED', human_decision: { action: 'DISMISS', dismissal_type: 'system_auto_dismiss', auto_dismiss_reason: 'portal_failed_auto_dismiss' }, human_decided_by: 'system' };
      }
      break;
    }
    ...
    case CaseEvent.PORTAL_TIMED_OUT: {
      ...
      m.portal_tasks = { id: context.portalTaskId, status: 'CANCELLED', error: context.error };
      if (context.proposalId) {
        m.proposals = { id: context.proposalId, status: 'DISMISSED', human_decision: { action: 'DISMISS', dismissal_type: 'system_auto_dismiss', auto_dismiss_reason: 'portal_timed_out_auto_dismiss' }, human_decided_by: 'system' };
      }
      break;
    }

    case CaseEvent.PORTAL_ABORTED: {
      m.cases = {
        status: 'needs_human_review',
        requires_human: true,
        pause_reason: context.pauseReason || 'PORTAL_ABORTED',
        ...(context.substatus ? { substatus: context.substatus } : {}),
      };
      if (context.proposalId) {
        m.proposals = { id: context.proposalId, status: 'DISMISSED' };
      ...
      if (context.portalTaskId) {
        m.portal_tasks = {
          id: context.portalTaskId,
          status: 'CANCELLED',
          error: context.error || 'PORTAL_ABORTED',
          completed_at: new Date().toISOString(),
        };
      }
      break;
    ...
    case CaseEvent.CASE_SENT: {
      ...
      // If a portal flow was running while a fallback channel succeeded
      // (e.g. PDF email), cancel active portal tasks to prevent late writes
      // from flipping the case back to a review state.
    ...
    case CaseEvent.CASE_ESCALATED: {
      const ESCALATION_STATUSES = ['needs_human_review', 'needs_phone_call', 'pending_fee_decision', 'needs_rebuttal'];
      const target = context.targetStatus || 'needs_human_review';
      if (!ESCALATION_STATUSES.includes(target)) {
        throw new Error(`CASE_ESCALATED: invalid targetStatus "${target}"`);
      }
      m.cases = {
        status: target,
        requires_human: true,
        pause_reason: context.pauseReason || 'UNSPECIFIED',
        ...(context.substatus !== undefined ? { substatus: context.substatus } : {}),
        ...(context.escalationReason ? { escalation_reason: context.escalationReason } : {}),
      };
      break;
    }
    ...
    case CaseEvent.ACKNOWLEDGMENT_RECEIVED:
    case CaseEvent.PARTIAL_DELIVERY_RECEIVED: {
      m.cases = { status: 'awaiting_response', requires_human: false, pause_reason: null };
      Object.assign(m, proposalDismissForStatus('awaiting_response'));
      break;
    }

    case CaseEvent.CASE_ID_STATE: {
      m.cases = {
        status: 'id_state',
        requires_human: true,
        ...(context.substatus ? { substatus: context.substatus } : {}),
    ...
    case CaseEvent.CASE_RESPONDED: {
      m.cases = {
        status: 'responded',
        requires_human: false,
        pause_reason: null,
        last_response_date: context.lastResponseDate || new Date().toISOString(),
        ...(context.substatus ? { substatus: context.substatus } : {}),
      };
      Object.assign(m, proposalDismissForStatus('responded'));
      break;
    }

    case CaseEvent.FEE_QUOTE_RECEIVED: {
      m.cases = {
        status: 'needs_human_fee_approval',
        requires_human: true,
        pause_reason: 'FEE_QUOTE',
        ...(context.substatus ? { substatus: context.substatus } : {}),
        ...(context.feeQuoteAmount !== undefined ? { last_fee_quote_amount: context.feeQuoteAmount } : {}),
        ...(context.feeQuoteCurrency ? { last_fee_quote_currency: context.feeQuoteCurrency } : {}),
        ...(context.feeQuoteAt ? { last_fee_quote_at: context.feeQuoteAt } : {}),
      };
      break;
    }

    case CaseEvent.CASE_RECONCILED: {
      const targetStatus = context.targetStatus || snapshot.caseData?.status;
      m.cases = {
        status: targetStatus,
        requires_human: false,
        pause_reason: null,
        ...(context.substatus !== undefined ? { substatus: context.substatus } : {}),
        ...(context.daysOverdue !== undefined ? { days_overdue: context.daysOverdue } : {}),
      };
      Object.assign(m, proposalDismissForStatus(targetStatus));
      break;
    }
  ...
  // Auto-align followups when case status changes (mirrors updateCaseStatus behavior).
  // Don't override explicit followup mutations set by the event handler.
  if (m.cases?.status && !m.followups) {
    const fa = followupAlignmentForStatus(m.cases.status);
    if (fa.followups) m.followups = fa.followups;
  }
