...
function computeMutations(snapshot, event, context = {}) {
  ...
  switch (event) {
    ...
    case CaseEvent.PORTAL_FAILED: {
      ...
      if (context.proposalId) {
        m.proposals = { id: context.proposalId, status: 'DISMISSED', human_decision: JSON.stringify({ action: 'DISMISS', reason: 'portal_failed_auto_dismiss' }) };
      }
      break;
    }
    ...
    case CaseEvent.PORTAL_TIMED_OUT: {
      ...
      if (context.proposalId) {
        m.proposals = { id: context.proposalId, status: 'DISMISSED', human_decision: JSON.stringify({ action: 'DISMISS', reason: 'portal_timed_out_auto_dismiss' }) };
      }
      break;
    }
